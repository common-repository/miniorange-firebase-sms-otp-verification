		...
		public function render() {
			switch ( $this->type ) {
				...
				case 'NOTICE':
					...
					'<p>' . wp_kses( mofr_( $this->message ), MoUtility::mo_allow_html_array() ) . '</p>' .
					'</div>';
					break;
				case 'ERROR':
					echo '<div style="margin-top:1%;"' .
					'class="notice mofr-notice-error notice-error is-dismissible mofr-admin-notif">' .
					'<p>' . wp_kses( mofr_( $this->message ), MoUtility::mo_allow_html_array() ) . '</p>' .
					'</div>';
					break;
				case 'SUCCESS':
					echo '<div  style="margin-top:1%;"' .
					'class="notice mofr-notice-success notice-success is-dismissible mofr-admin-notif">' .
					'<p>' . wp_kses( mofr_( $this->message ), MoUtility::mo_allow_html_array() ) . '</p>' .
					'</div>';
					break;
			}
