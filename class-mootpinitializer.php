...
if ( ! class_exists( 'MoOTPInitializer' ) ) {
	...
	final class MoOTPInitializer {
		...
		private function initialize_hooks() {
			...
			add_action( 'plugin_action_links_' . MOFLR_PLUGIN_NAME, array( $this, 'plugin_action_links' ), 10, 1 );
		...
		public function plugin_action_links( $links ) {
			...
			$form_settings_tab = $tab_details->tab_details[ Tabs::LOGINREGISTRATIONFORMS ];
			if ( ! function_exists( 'is_plugin_active' ) ) {
				include_once ABSPATH . 'wp-admin/includes/plugin.php';
			}
			if ( is_plugin_active( MOFLR_PLUGIN_NAME ) ) {
				$links = array_merge(
					array(
