	...
	class LoginForm extends FormHandler implements IFormHandler {
		...
		public function mofr_validate_otp_token() {
			...
			if ( ! isset( $post['id_token'] ) ) {
				wp_send_json_error( 'Missing ID token' );
				return;
			}

			$id_token        = sanitize_text_field( $post['id_token'] );
			$gateway_api_key = get_mofr_option( 'gateway_apiKey' );

			$firebase_url = 'https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=' . $gateway_api_key;

			$response = MocURLOTP::mofr_call_api( $firebase_url, wp_json_encode( array( 'idToken' => $id_token ) ) );

			$data = json_decode( $response, true );

			if ( isset( $data['users'][0] ) ) {
				$user = $this->getUserFromPhoneNumber( sanitize_text_field( $data['users'][0]['phoneNumber'] ) );
				if ( MoPHPSessions::get_session_var( 'mofr_login_phone_verified' ) !== $data['users'][0]['phoneNumber'] || MoPHPSessions::get_session_var( 'mofr_login_phone_verified' ) !== $post['user_phone'] ) {
					...
					MoPHPSessions::unset_session( 'mofr_login_phone_verified' );
					wp_set_auth_cookie( $user->data->ID );
		...
		public function mofr_validate_reset_pass_otp_token() {
			...
			$id_token = sanitize_text_field( $post['id_token'] );
			$email    = sanitize_text_field( $post['user_phone'] );
			if ( MoPHPSessions::get_session_var( 'mofr_reset_pass_email_verified' ) !== $email ) {
				...
				MoPHPSessions::unset_session( 'mofr_reset_pass_email_verified' );
				MoPHPSessions::add_session_var( 'mofr_reset_pass_otp_id_token', $id_token );
				wp_send_json( MoUtility::create_json( 'otp_validated_successfully', 'success' ) );
			}
		}
		/**
		...
		public function reset_password() {
			...
			$id_token        = MoPHPSessions::get_session_var( 'mofr_reset_pass_otp_id_token' );
			$gateway_api_key = get_mofr_option( 'gateway_apiKey' );
			...
			if ( isset( $data['users'][0] ) ) {
				...
				$user     = $this->getUserFromPhoneNumber( $data['users'][0]['phoneNumber'] );
				...
				if ( $new_pass !== $confirm_pass ) {
					...
					MoPHPSessions::unset_session( 'mofr_reset_pass_otp_id_token' );
					wp_send_json( MoUtility::create_json( 'Password updated succesfully!', 'success' ) );
				}
			} else {
				wp_send_json( MoUtility::create_json( 'Some Internal Error Occured', 'error' ) );
			}
		}
		/**
		...
		public function miniorange_login_mofirebaselogin_script() {
			$page_details = get_posts(
				array(
					'title'     => get_mofr_option( 'mofr_login_form_redirecturl' ),
					'post_type' => 'any',
				)
			);
			$page_id      = $page_details[0];
			$page_id      = $page_id->ID;
		...
		public function miniorange_reset_password_script() {
			...
			wp_localize_script(
				...
				array(
					...
					'redirecturl' => get_page_link( get_mofr_option( 'mofr_login_form_redirecturl' ) ),
				)
			);
		...
		public function handle_mofirebase_form_options() {
			...
			if ( file_exists( MOFLR_DIR . 'helper/class-mofirebasepremiumplan.php' ) ) {
				update_mofr_option( 'mofr_login_form_redirecturl', $this->redirecturl );
			}
		}
	}
}
