...
if ( ! class_exists( 'RegistrationForm' ) ) {
	...
	class RegistrationForm extends FormHandler implements IFormHandler {
		...
		public function mofr_validate_otp_token() {
			...
			MoPHPSessions::add_session_var( 'is_regform_id_token', $id_token );
		...
		public function validate_form_submission() {
			...
			if ( $this->is_form_enabled ) {
				$id_token        = MoPHPSessions::get_session_var( 'is_regform_id_token' );
				...
				if ( isset( $data['users'][0] ) ) {
					if ( ! MoPHPSessions::get_session_var( 'mo_registration_phone_verified' ) || ! isset( $data['users'][0]['phoneNumber'] ) ) {
						wp_send_json( MoUtility::create_json( 'Please verify yourself before submitting the form', 'error' ) );
					} elseif ( MoPHPSessions::get_session_var( 'mo_registration_phone_verified' ) !== sanitize_text_field( $post['user_phone'] ) || strcmp( $data['users'][0]['phoneNumber'], $post['user_phone'] ) !== 0 ) {
						wp_send_json( MoUtility::create_json( 'Phone number on which OTP was sent and phone submitted does not match', 'error' ) );
					}
				} else {
					wp_send_json( MoUtility::create_json( 'Please verify yourself before submitting the form', 'error' ) );
				}
			...
			$userdata = array(
				...
				'role'       => get_mofr_option( 'default_user_role' ) ? get_mofr_option( 'default_user_role' ) : 'subscriber',
			);
			...
			MoPHPSessions::unset_session( 'mo_registration_phone_verified' );
			MoPHPSessions::unset_session( 'is_regform_id_token' );
		...
		public function miniorange_register_mofirebasereg_script() {
			$page_details = get_posts(
				array(
					'title'     => get_mofr_option( 'mofr_reg_form_redirecturl' ),
					'post_type' => 'any',
				)
			);
		...
		public function handle_mofirebase_form_options() {
			...
			if ( file_exists( MOFLR_DIR . 'helper/class-mofirebasepremiumplan.php' ) ) {
				update_mofr_option( 'mofr_reg_form_redirecturl', $this->redirecturl );
